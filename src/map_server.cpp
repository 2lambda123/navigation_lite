...
public:
  explicit MapServer(const rclcpp::NodeOptions &options)
    ...
    this->declare_parameter<double>(
        "map_resolution",
        0.1); // Spatial extent of tree is 6553,6 meters in either direction

    max_range_ = this->declare_parameter<double>(
        "max_range", -1.0); // Max range (m) when inserting data into map
    insert_depth_ =
        this->declare_parameter<int>("insert_depth", 0); // Integration depth
    simple_ray_casting_ =
        this->declare_parameter<bool>("simple_ray_casting", false);
    early_stopping_ =
        this->declare_parameter<int>("early_stopping", 0);  // Early Stopping
    ...
    robot_height_ =
        this->declare_parameter<double>("robot_height", 0.4); // Robot height(m)
    robot_radius_ =
        this->declare_parameter<double>("robot_radius", 0.5); // Robot radius(m)
    clearing_depth_ =
        this->declare_parameter<int>("clearing_depth", 0); // Clearing depth

    // Kick off a init routine
    this->init_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(500), std::bind(&MapServer::init, this));
  }
...
private:
  ...
  rclcpp::Publisher<navigation_interfaces::msg::UfoMapStamped>::SharedPtr
      map_publisher_;
  ...
  void init() {
    ...
    map_publisher_ =
        this->create_publisher<navigation_interfaces::msg::UfoMapStamped>(
            map_topic_, 3);
    ...
    subscription_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        ...
        std::bind(&MapServer::topic_callback, this, std::placeholders::_1));

    pub_timer_ = this->create_wall_timer(
        1000ms, std::bind(&MapServer::publish_map, this));
    // publish_map();  // Send the first map, and then only when it has been
    // updated.

    // Create simple services
    load_service = this->create_service<navigation_interfaces::srv::LoadMap>(
        "nav_lite/load_map", std::bind(&MapServer::load_map, this, _1, _2));
    save_service = this->create_service<navigation_interfaces::srv::SaveMap>(
        "nav_lite/save_map", std::bind(&MapServer::save_map, this, _1, _2));
    reset_service = this->create_service<navigation_interfaces::srv::Reset>(
        "nav_lite/reset_map", std::bind(&MapServer::reset_map, this, _1, _2));
  }
  ...
  void publish_map() {
    ...
    auto message =
        std::make_shared<navigation_interfaces::msg::UfoMapStamped>();
    // navigation_interfaces::msg::UfoMapStamped::Ptr message(new
    // navigation_interfaces::msg::UfoMapStamped); Convert UFOMap to ROS Message
    if (ufomap_msgs::ufoToMsg(*map_, message->map,
                              ufo::geometry::BoundingVolume(), compress,
                              pub_depth, 1, 0)) {
              ...
              ufo::map::PointCloudColor& cloud_out)
  {
    ...
    if (!has_x || !has_y || !has_z) {
        throw std::runtime_error("cloud_in missing one or more of the xyz
  fields");
    }
  ...
  !std::isnan(*iter_z) && !std::isnan(*iter_r) && !std::isnan(*iter_g) &&
  !std::isnan(*iter_b)) { cloud_out.push_back( ufo::map::Point3Color(*iter_x,
  *iter_y, *iter_z, *iter_r, *iter_g, *iter_b));
                }
        }

    } else {
        // TODO: Should this throw?
        // throw std::runtime_error("cloud_in missing one or more of the rgb
  fields");

        for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z) {
                if (!std::isnan(*iter_x) && !std::isnan(*iter_y) &&
  !std::isnan(*iter_z)) { cloud_out.push_back(ufo::map::Point3Color(*iter_x,
  *iter_y, *iter_z));
                }
        }
    }
  ...
  */
  void topic_callback(sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    try {
      ...
      geometry_msgs::msg::TransformStamped tf_trans =
          tf_buffer_->lookupTransform(map_frame_id_, msg->header.frame_id,
                                      tf2::TimePointZero);
      // Convert ROS transform to UFO transform
      transform = ufomap_ros::rosToUfo(tf_trans.transform);

    } catch (tf2::TransformException &ex) {
      RCLCPP_WARN(this->get_logger(), "Could not transform %s to %s: %s", "map",
                  msg->header.frame_id, ex.what());
      return;
    }
    ...
    // Transform point cloud to correct frame, do it in parallel (second param
    // true)
    ...
    map_->insertPointCloudDiscrete(transform.translation(), cloud, max_range_,
    ...
    if (clear_robot_) {
      try {
        // Lookup transform
        geometry_msgs::msg::TransformStamped tf_trans =
            tf_buffer_->lookupTransform(map_frame_id_, robot_frame_id_,
                                        tf2::TimePointZero);
        // Convert ROS transform to UFO transform
        transform = ufomap_ros::rosToUfo(tf_trans.transform);
      ...
      // map_->setValueVolume(aabb, map_->getClampingThresMin(),
      // clearing_depth_);  //
      // /usr/local/include/ufo/map/occupancy_map_base.h:508:4: error: call of
      // overloaded ‘setOccupancy(float&, double)’ is ambiguous
    }
  ...
  bool save_map(
      const std::shared_ptr<navigation_interfaces::srv::SaveMap::Request>
          request,
      std::shared_ptr<navigation_interfaces::srv::SaveMap::Response> response) {
    //    response->success = map_->write(request->filename, false,
    //    request->depth);

    ufo::geometry::BoundingVolume bv =
        ufomap_msgs::msgToUfo(request->bounding_volume);
    response->success =
        map_->write(request->filename, bv, request->compress, request->depth, 1,
                    request->compression_level);
    return true;
  }

  void load_map(
      const std::shared_ptr<navigation_interfaces::srv::LoadMap::Request>
          request,
  ...
  void reset_map(
      const std::shared_ptr<navigation_interfaces::srv::Reset::Request> request,
