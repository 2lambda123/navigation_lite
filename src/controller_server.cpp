...
static const float DEFAULT_MAX_SPEED_XY =
    2.0; // Maximum horizontal speed, in m/s
static const float DEFAULT_MAX_ACCEL_XY =
    0.2; // Maximum horizontal acceleration, in m/s/s
static const float DEFAULT_MAX_SPEED_Z = 0.33; // Maximum vertical speed, in m/s
static const float DEFAULT_MAX_YAW_SPEED =
    0.5; // Maximum yaw speed in radians/s
static const float DEFAULT_WAYPOINT_RADIUS_ERROR =
    0.3; // Acceptable XY distance to waypoint deemed as close enough
static const float DEFAULT_YAW_THRESHOLD =
    0.025; // Acceptible YAW to start foreward acceleration
static const float DEFAULT_ALTITUDE_THRESHOLD =
    0.3; // Acceptible Z distance to altitude deemed as close enough
static const int DEFAULT_HOLDDOWN =
    2; // Time to ensure stability in flight is attained
...
public:
  ...
  using GoalHandleFollowWaypoints =
      rclcpp_action::ServerGoalHandle<FollowWaypoints>;
...
private:
  std::mutex server_mutex; // Only allow one Action Server to address the drone
                           // at a time
  ...
  void init() {
    ...
    freq_ = this->declare_parameter(
        "frequency",
        10.0); // Control frequency in Hz.  Must be bigger than 2 Hz
    ...
    waypoint_radius_error_ = this->declare_parameter<float>(
        "waypoint_radius_error", DEFAULT_WAYPOINT_RADIUS_ERROR);
    yaw_threshold_ =
        this->declare_parameter<float>("yaw_threshold", DEFAULT_YAW_THRESHOLD);
    altitude_threshold_ = this->declare_parameter<float>(
        "altitude_threshold", DEFAULT_ALTITUDE_THRESHOLD);
    holddown_ = this->declare_parameter<int>("holddown", DEFAULT_HOLDDOWN);

    map_frame_ = this->declare_parameter<std::string>("map_frame", "map");
    drone_diameter_ = this->declare_parameter<double>(
        "drone_diameter", 0.80); // 800 mm for my current craft.

    // The grid size of the map is still hard coded, thus this is treated as a
    // constant
    yaw_control_limit_ = 1.0; // Distance from waypoint where control moves to X
                              // and Y PID rather than Yaw and X
    ...
    std::vector<double> pid_xy_settings =
        pid_xy_settings_param.as_double_array();
    pid_x = std::make_shared<PID>(
        1.0 / freq_, max_speed_xy_, -max_speed_xy_, (float)pid_xy_settings[0],
        (float)pid_xy_settings[1], (float)pid_xy_settings[2]);
    pid_y = std::make_shared<PID>(
        1.0 / freq_, max_speed_xy_, -max_speed_xy_, (float)pid_xy_settings[0],
        (float)pid_xy_settings[1], (float)pid_xy_settings[2]);
    ...
    pid_z = std::make_shared<PID>(
        1.0 / freq_, max_speed_z_, -max_speed_z_, (float)pid_z_settings[0],
        (float)pid_z_settings[1], (float)pid_z_settings[2]);
    ...
    std::vector<double> pid_yaw_settings =
        pid_yaw_settings_param.as_double_array();
    pid_yaw = std::make_shared<PID>(1.0 / freq_, max_yaw_speed_,
                                    -max_yaw_speed_, (float)pid_yaw_settings[0],
                                    (float)pid_yaw_settings[1],
                                    (float)pid_yaw_settings[2]);
    ...
    resolution = this->declare_parameter<double>(
        "map_resolution",
        0.25); // use resolution 0.25.  Can then query the map at 0.5 and 1.0
    map_ = std::make_shared<ufo::map::OccupancyMap>(resolution);

    subscription_ =
        this->create_subscription<navigation_interfaces::msg::UfoMapStamped>(
            "nav_lite/map", 10,
            std::bind(&ControllerServer::topic_callback, this, _1));

    // Create the action server
    this->action_server_ = rclcpp_action::create_server<FollowWaypoints>(
        this, "nav_lite/follow_waypoints",
    ...
    RCLCPP_INFO(this->get_logger(),
                "Action Server [nav_lite/follow_waypoints] started");
  }

  // MAP SUBSCRIPTION
  // ///////////////////////////////////////////////////////////////////////////////////////////////
  void topic_callback(
      const navigation_interfaces::msg::UfoMapStamped::SharedPtr msg) const {
  ...
  rclcpp::Subscription<navigation_interfaces::msg::UfoMapStamped>::SharedPtr
      subscription_;
  ...
  bool fly_to_waypoint(geometry_msgs::msg::PoseStamped wp) {
    ...
    do {
      ...
      setpoint.angular.z =
          pid_yaw->calculate(0, -yaw_error); // correct yaw error down to zero

      publisher_->publish(setpoint);
      loop_rate.sleep(); // Give the drone time to move
    } while (!pose_is_close_);

    // Now that we are ponting, keep on adjusting yaw, but include altitude and
    // foreward velocity
    ...
    do {
      ...
      // Govern acceleration, and decellaration.  The latter should be governed
      // by a well tuned PID but then not all control is done via a PID.  Often
      // velocity is forced to 0 which is an abrupt stop.
      if (setpoint.linear.x > last_v_x) { // Acceleration
        setpoint.linear.x =
            min(setpoint.linear.x, last_v_x + (max_accel_xy_ / freq_));
      } else { // Decelleration
        setpoint.linear.x =
            max(setpoint.linear.x, last_v_x - (max_accel_xy_ / freq_));
      }
      if (setpoint.linear.y > last_v_y) {
        setpoint.linear.y =
            min(setpoint.linear.y, last_v_y + (max_accel_xy_ / freq_));
      } else {
        setpoint.linear.y =
            max(setpoint.linear.y, last_v_y - (max_accel_xy_ / freq_));
      }
  ...
  handle_goal(const rclcpp_action::GoalUUID &uuid,
    ...
    RCLCPP_INFO(this->get_logger(), "Received request to follow %d waypoints",
                goal->poses.size());
    (void)uuid;
    if (server_mutex.try_lock()) {
      ...
      RCLCPP_ERROR(
          this->get_logger(),
          "Another thread is commanding the drone now.  Rejecting request.");
  ...
  void handle_accepted(
    ...
    // new thread
    std::thread{std::bind(&ControllerServer::execute, this, _1), goal_handle}
        .detach();
  }
  ...
  void execute(const std::shared_ptr<GoalHandleFollowWaypoints> goal_handle) {
    ...
    current_waypoint = 0;
    unsigned int last_reached_waypoint =
        std::numeric_limits<unsigned int>::max();

    // Start at the goal, and search through all the waypoints till the first
    // one that I can fly to without an obstacle.  If nothing is found, fail
    // gracefully back.  If one is found, fly straight to it, without going
    // through all the other waypoints.
    while ((current_waypoint < goal->poses.size()) && (rclcpp::ok())) {
      ...
      // Optimistically search for a next waypoint that has a clear path from
      ...
      for (size_t i = current_waypoint; i < goal->poses.size(); i++) {
        ...
        if (isPathClear(wp.pose.position.x, wp.pose.position.y,
                        wp.pose.position.z)) {
    ...
    if (rclcpp::ok()) {
      if (last_reached_waypoint == (goal->poses.size() - 1)) {
        // Mission complete.  Turn the drone to the pose offered by the final
        // waypoint
        correct_yaw(goal->poses.back());
      } else {
        //  Missed some goals, hence the route failed. (Could not find a clear
        //  path to next waypoint)
        for (size_t i = last_reached_waypoint + 1; i < goal->poses.size();
             i++) {
          result->missed_waypoints.push_back(i);
        }
      }
  ...
  bool isPathClear(const float x, const float y, const float z) {
    ...
    RCLCPP_DEBUG(this->get_logger(),
                 "Drone at %.2f,%.2f,%.2f requested to move to %.2f,%.2f,%.2f ",
                 cx, cy, cz, x, y, z);
    ...
    bool horizontal = sqrt(pow(err_x,2) + pow(err_y,2)) >
    waypoint_radius_error_; bool vertical = fabs(cz - z) > altitude_threshold_;
    ...
    */
    ...
    // Check if it is possible to move between the robot's current position and
    // goal
    ...
    // Check if the oriented bounding box collides with either occupied or
    // unknown space, at depth 3 (16 cm)
    if (isInCollision(map_, obb, true, false, false, 3)) {
      return false;
      // std::cout << "The path between position and goal is not clear" <<
      // std::endl;
    } else {
      // std::cout << "It is possible to move between position and goal in a
      // straight line" << std::endl;
    }
