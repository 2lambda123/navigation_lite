...
inline double getDiff2Angles(const double x, const double y, const double c) {
  ...
  double sign = (x - y >= 0.0 && x - y <= c) || (x - y <= -c && x - y > -2 * c)
                    ? 1.0
                    : -1.0;
  return sign * r;
}
...
public:
  ...
  explicit RecoveryServer(
      const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
      : Node("recovery_server", options) {
    one_off_timer_ =
        this->create_wall_timer(1000ms, std::bind(&RecoveryServer::init, this));
  }
...
private:
  ...
  std::shared_ptr<geometry_msgs::msg::Pose> last_pose =
  ...
  spin_handle_goal(const rclcpp_action::GoalUUID &uuid,
                   std::shared_ptr<const Spin::Goal> goal) {
    RCLCPP_DEBUG(this->get_logger(),
                 "Received request to rotate to %.2f radians",
                 goal->target_yaw);
    (void)uuid;
    if (server_mutex.try_lock()) {
      return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
    } else {
      RCLCPP_ERROR(this->get_logger(), "Another thread is commanding the drone "
                                       "now.  Rejecting Spin request.");
      return rclcpp_action::GoalResponse::REJECT;
    }
  }

  rclcpp_action::CancelResponse
  spin_handle_cancel(const std::shared_ptr<GoalHandleSpin> goal_handle) {
  ...
  void spin_handle_accepted(const std::shared_ptr<GoalHandleSpin> goal_handle) {
    ...
    // new thread
    std::thread{std::bind(&RecoveryServer::spin_execute, this, _1), goal_handle}
        .detach();
  }
  ...
  rclcpp_action::GoalResponse
  wait_handle_goal(const rclcpp_action::GoalUUID &uuid,
                   std::shared_ptr<const Wait::Goal> goal) {
    RCLCPP_DEBUG(this->get_logger(),
                 "Received request to wait for %d seconds and %d nanoeconds",
                 goal->time.sec, goal->time.nanosec);
    (void)uuid;
    if (server_mutex.try_lock()) {
      return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
    } else {
      RCLCPP_ERROR(this->get_logger(), "Another thread is commanding the drone "
                                       "now.  Rejecting Wait request.");
      return rclcpp_action::GoalResponse::REJECT;
    }
  }

  rclcpp_action::CancelResponse
  wait_handle_cancel(const std::shared_ptr<GoalHandleWait> goal_handle) {
  ...
  void wait_handle_accepted(const std::shared_ptr<GoalHandleWait> goal_handle) {
    ...
    // this needs to return quickly to avoid blocking the executor, so spin up a
    // new thread
    std::thread{std::bind(&RecoveryServer::wait_execute, this, _1), goal_handle}
        .detach();
  }
  ...
  void wait_execute(const std::shared_ptr<GoalHandleWait> goal_handle) {
    ...
    while (rclcpp::ok() && keep_on_waiting) {
      ...
      goal_handle->publish_feedback(feedback);
      RCLCPP_DEBUG(this->get_logger(), "Time left %d sec %d nanosec",
                   time_left.sec, time_left.nanosec);
      keep_on_waiting = ((time_left.sec > 0) &&
                         (time_left.nanosec >
                          500000)); // remember a loop_rate.sleep() still comes!
