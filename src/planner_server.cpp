...
public:
  ...
  using GoalHandleComputePathToPose =
      rclcpp_action::ServerGoalHandle<ComputePathToPose>;
  ...
  explicit PlannerServer(
    ...
    subscription_ =
        this->create_subscription<navigation_interfaces::msg::UfoMapStamped>(
            "nav_lite/map", 10,
            std::bind(&PlannerServer::topic_callback, this, _1));

    this->planning_action_server_ =
        rclcpp_action::create_server<ComputePathToPose>(
            this, "nav_lite/compute_path_to_pose",
    ...
    RCLCPP_INFO(this->get_logger(),
                "Action Server [nav_lite/compute_path_to_pose] started");
  }
...
private:
  ...
  handle_plan_goal(const rclcpp_action::GoalUUID &uuid,
                   std::shared_ptr<const ComputePathToPose::Goal> goal) {
    RCLCPP_DEBUG(this->get_logger(),
                 "Received goal request for path to [%.2f;%.2f;%.2f]",
                 goal->goal.pose.position.x, goal->goal.pose.position.y,
                 goal->goal.pose.position.z);
    (void)uuid;

    if (((goal->goal.pose.position.x < (-e_size_ / 2)) ||
         (goal->goal.pose.position.x > (e_size_ / 2))) ||
        ((goal->goal.pose.position.y < (-n_size_ / 2)) ||
         (goal->goal.pose.position.y > (n_size_ / 2))) ||
        ((goal->goal.pose.position.z < 0) ||
         (goal->goal.pose.position.z > u_size_))) {
      RCLCPP_ERROR(this->get_logger(),
                   "Goal of [%.2f;%.2f;%.2f] is outside map range. [%i;%i;%i]",
                   goal->goal.pose.position.x, goal->goal.pose.position.y,
                   goal->goal.pose.position.z, e_size_, n_size_, u_size_);
  ...
  void handle_plan_accepted(
    ...
    // new thread
    std::thread{std::bind(&PlannerServer::execute_plan, this, _1), goal_handle}
        .detach();
  }

  void
  execute_plan(const std::shared_ptr<GoalHandleComputePathToPose> goal_handle) {
    ...
    if (bypass_planning_) {
      ...
      // # If false, use current robot pose as path start, if true, use start
      // above instead
      if (goal->use_start == true) {
        RCLCPP_DEBUG(this->get_logger(),
                     "Planning a path from %.2f, %.2f, %.2f",
                     goal->start.pose.position.x, goal->start.pose.position.y,
                     goal->start.pose.position.z);
        dsl->setStart((int)goal->start.pose.position.x,
                      (int)goal->start.pose.position.y,
                      (int)goal->start.pose.position.z);
      } else {
        // use the current robot position.
        float x, y, z;
        read_position(&x, &y, &z); // From tf2
        dsl->setStart(x, y, z);
        RCLCPP_INFO(this->get_logger(), "Planning a path from %.2f, %.2f, %.2f",
                    x, y, z);
      }

      dsl->setGoal((int)goal->goal.pose.position.x,
                   (int)goal->goal.pose.position.y,
                   (int)goal->goal.pose.position.z);
      dsl->initialize();
      ...
      if (result->path.poses.size() > 0) {
        ...
        result->path.poses.back().pose.orientation.x =
            goal->goal.pose.orientation.x;
        result->path.poses.back().pose.orientation.y =
            goal->goal.pose.orientation.y;
        result->path.poses.back().pose.orientation.z =
            goal->goal.pose.orientation.z;
        result->path.poses.back().pose.orientation.w =
  ...
  bool isOccupied(const float x, float y, float z) {
    // Would it be better to see if the robot can travel between current
    // position (tf_listener) and the target posiiton ina straight line.  Then
    // the granularity of my d*lite algoritm and the UFO map changes a bit.
    ...
    for (auto it = map_->beginLeaves(
                  sphere, true, false, false, false,
                  2), // Use resolution of 0->0.25m, 1->0.5m 2->1.0m
