...
public:
  ...
  explicit NavigationServer(
    ...
    subscription_ = this->create_subscription<sensor_msgs::msg::BatteryState>(
        "drone/battery", 5,
        std::bind(&NavigationServer::battery_callback, this,
                  std::placeholders::_1));
    ...
    this->action_server_ = rclcpp_action::create_server<NavigateToPose>(
        this, "nav_lite/navigate_to_pose",
    ...
    RCLCPP_INFO(this->get_logger(),
                "Action Serever [nav_lite/navigate_to_pose] started");
  }
...
private:
  ...
  BT::NodeStatus CheckBattery() {
    return (current_battery_voltage_ >= minimum_battery_voltage_)
  ...
  handle_goal(const rclcpp_action::GoalUUID &uuid,
    ...
    RCLCPP_DEBUG(this->get_logger(), "Received request with behaviour tree %s",
                 goal->behavior_tree.c_str());
    RCLCPP_DEBUG(this->get_logger(),
                 "Received goal request to fly to [%.2f; %.2f; %.2f]",
  ...
  handle_accepted(const std::shared_ptr<GoalHandleNavigateToPose> goal_handle) {
    ...
    // new thread
    std::thread{std::bind(&NavigationServer::execute, this, _1), goal_handle}
        .detach();
  }

  /* std::string pose3D_string(Pose3D bt_goal_msg) {
    std::stringstream ss;
    ss << bt_goal_msg.x << ";" << bt_goal_msg.y << ";" << bt_goal_msg.z << ";"
  << bt_goal_msg.theta; return ss.str();
  }
  */
  ...
  void execute(const std::shared_ptr<GoalHandleNavigateToPose> goal_handle) {
    ...
    factory.registerSimpleCondition(
        "BatteryOK", std::bind(&NavigationServer::CheckBattery, this));
    ...
    factory.registerNodeType<NavLiteReadGoalAction>(
        "ReadGoal"); //  10,0;1,0;5,0;0.0
    ...
    factory.registerNodeType<NavLiteComputePathToPoseAction>(
        "ComputePathToPose");
    ...
    for (auto &node : tree.nodes) {
      ...
      if (auto read_goal_action =
              dynamic_cast<NavLiteReadGoalAction *>(node.get())) {
        read_goal_action->init(node_ptr, bt_goal_msg);
      } else if (auto wait_action =
                     dynamic_cast<NavLiteWaitAction *>(node.get())) {
        wait_action->init(node_ptr);
      } else if (auto spin_action =
                     dynamic_cast<NavLiteSpinAction *>(node.get())) {
        spin_action->init(node_ptr);
      } else if (auto follow_waypoints_action =
                     dynamic_cast<NavLiteFollowWaypointsAction *>(node.get())) {
        follow_waypoints_action->init(node_ptr);
      } else if (auto compute_path_to_pose_action =
                     dynamic_cast<NavLiteComputePathToPoseAction *>(
                         node.get())) {
    ...
    while ((tree.tickRoot() == NodeStatus::RUNNING) && rclcpp::ok()) {
      ...
      navigation_time =
          now() - start_time; // builtin_interfaces/Duration navigation_time
      estimated_time_remaining.sec =
          0; // builtin_interfaces/Duration estimated_time_remaining
      ...
      distance_remaining = sqrt(pow(err_z, 2) + pow(err_x, 2) +
                                pow(err_y, 2)); // float32 distance_remaining
